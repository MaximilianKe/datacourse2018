...
```
```{sql connection=con}
select * from dist_to_herd where distance_toherdabau <15000
```
```{sql connection=con}
...
```
```{sql connection=con}
select * from dist_to_muns where distance_tomunster <1500
```
Comment: I think it is strange HOBO stations do not appear in this query. I don't understand why only BW stations appear as a result. 
...
select d.name, round((sum(st_area(vc.geometry))/1000000)) as tot_area_Km2 from vegetation_cover vc join vegetation_cover_description d on vc.description_id=d.id
group by d.name
```
```{sql connection=con}
select * from tot_area_v 
```

```{sql connection=con}
select name, tot_area_Km2, round(tot_area_Km2*100/sum) as Ratio from tot_area_v, (SELECT SUM(tot_area_Km2) FROM tot_area_v AS amount) t
```
...
Answer: Small differences ammong the first two coordinates systems are due to difference in precition for the location. The bigger difference in the WGS84 is because this coordinate system is in degrees, so, the result is not in meters and can not be compared with the two other coordinate system, which are in meters. 
</div>
