...
```{sql connection=con}
select name, description_id, sum(area) / 1000000 as "area [km2]" from
...
```  
```{sql connection=con}  
select name, description_id, area, round(cast((area/(select sum(area) from share_lk))*100 as numeric),1) as ratio from share_lk
```
...
```{sql connection=con}
select * from 
    (select st_union(geometry) as agg_geometry from einzugsgebiete) e
    join 
    (select st_union(geometry) as agg_geometry from vegetation_cover) vc
on st_within(vc.agg_geometry, e.agg_geometry) or st_within(e.agg_geometry, vc.agg_geometry)
```
...
Answer: They differ, because the different srid were used describing different coordinate systems and geodetic datum. The different mapping through this srids results in different distances in the query.
</div>
