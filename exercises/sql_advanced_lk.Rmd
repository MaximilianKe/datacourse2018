...
```{sql connection=con}
select id, extremes_m as extremes, month as date from lk_precipitation_monthly where extremes_m = (select max(extremes_m) from lk_precipitation_monthly)
union
select id, extremes_y as extremes, year as date from lk_precipitation_yearly where extremes_y = (select max(extremes_y) from lk_precipitation_yearly)
...
select  id, type_of_prec, count(type_of_prec) as amount from extr_prec_type_lk group by id, type_of_prec 
```
```{r}
hist(hist2$type_of_prec)
```
...
Answer: The variable shows the aggregate state of the precipitation. Therefore the measurements of measurement can be described more precisely. With the count(type_of_prec) the occurence and distribution per station of the different types can be shown.
